...
const InstanceSnapshotsFilters: React.FC<InstanceSnapshotsFiltersProps> = ({ snapshots, setFilteredSnapshots }) => {
  const filterConfig: FilterConfig<InstanceSnapshot> = useMemo(
    () => {
      // Build region-to-cloudProvider map for O(1) lookups
      const regionToCloudProviderMap = new Map<string, string>();
      snapshots.forEach((snapshot) => {
        if (snapshot.region && snapshot.cloudProvider && !regionToCloudProviderMap.has(snapshot.region)) {
          regionToCloudProviderMap.set(snapshot.region, snapshot.cloudProvider);
        }
      });
      ...
      return {
        ...
        region: {
          ...
          options: deriveOptionsFromData(snapshots, "region", (value) => {
            // Append Cloud Provider to Region using the map for O(1) lookup
            const cloudProvider = regionToCloudProviderMap.get(value);
            if (cloudProvider) {
